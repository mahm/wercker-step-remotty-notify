...
if [ ! -n "$WERCKER_REMOTTY_NOTIFY_FAILED_MESSAGE" ]; then
  if [ ! -n "$DEPLOY" ]; then
    ...
    export WERCKER_REMOTTY_NOTIFY_FAILED_MESSAGE="[label:danger:Failed daply to $WERCKER_STARTED_BY] **$WERCKER_APPLICATION_OWNER_NAME/$WERCKER_APPLICATION_NAME** [log]($WERCKER_DEPLOY_URL)"
  fi
fi
...
if [ ! -n "$WERCKER_REMOTTY_NOTIFY_PASSED_MESSAGE" ]; then
  if [ ! -n "$DEPLOY" ]; then
    ...
    export WERCKER_REMOTTY_NOTIFY_PASSED_MESSAGE="[label:success:Deployed to: $WERCKER_DEPLOYTARGET_NAME] **$WERCKER_APPLICATION_OWNER_NAME/$WERCKER_APPLICATION_NAME** [log]($WERCKER_DEPLOY_URL)"
  fi
fi

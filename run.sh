...
if [ ! -n "$WERCKER_REMOTTY_NOTIFY_PASSED_MESSAGE" ]; then
  if [ ! -n "$DEPLOY" ]; then
    export WERCKER_REMOTTY_NOTIFY_PASSED_MESSAGE="(\`･ω･) **$WERCKER_APPLICATION_OWNER_NAME/$WERCKER_APPLICATION_NAME**: [**PASSES** build of $WERCKER_GIT_BRANCH to $WERCKER_DEPLOYTARGET_NAME by $WERCKER_STARTED_BY]($WERCKER_DEPLOY_URL)"
  else
